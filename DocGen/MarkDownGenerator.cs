    ...
    internal class MarkDownGenerator
    {
        ...
        private void GenerateCmdletDocs()
        {
            ...
            foreach (var cmdletInfo in _cmdlets)
            {
                ...
                if (!string.IsNullOrEmpty(cmdletInfo.Verb) && !string.IsNullOrEmpty(cmdletInfo.Noun))
                {
                    ...
                    if (cmdletInfo.Platforms.Any())
                    {
                        platform = string.Join(", ", cmdletInfo.Platforms);
                    }
                    ...
                    if (cmdletInfo.Examples.Any())
                    {
                        ...
                        foreach (var example in cmdletInfo.Examples.OrderBy(e => e.SortOrder))
                        {
                            ...
                            var examplePlatforms = new List<string>();
                            if (example.Platforms.Any())
                            {
                                foreach (var examplePlatform in example.Platforms)
                                {
                                    if (!cmdletInfo.Platforms.Contains(examplePlatform))
                                    {
                                        examplePlatforms.Add(examplePlatform);
                                    }
                                }
                            }
                            if (examplePlatforms.Any())
                            {
                                docBuilder.Append($"APPLIES TO: {string.Join(", ", examplePlatforms)}{Environment.NewLine}");
                            }
        ...
        private void GenerateTOC()
        {
            ...
            foreach (var category in categories.OrderBy(c => c))
            {
                ...
                foreach (var cmdletInfo in _cmdlets.Where(c => c.Category == category).OrderBy(c => c.Noun))
                {
                    ...
                    docBuilder.AppendFormat("**[{0}]({1}-{2}.md)** |{3}|{4}{5}", cmdletInfo.FullCommand.Replace("-", "&#8209;"), cmdletInfo.Verb, cmdletInfo.Noun, description, cmdletInfo.Platforms, Environment.NewLine);
                }
            }
        ...
        private void GenerateMSDNLandingPage(string landingPagePath)
        {
            ...
            foreach (var category in categories.OrderBy(c => c))
            {
                ...
                {
                    var description = cmdletInfo.Description != null ? cmdletInfo.Description.Replace("\r\n", " ") : "";
                    docBuilder.AppendFormat("**[{0}](../../sharepoint-ps/sharepoint-pnp/{1}-{2}.md)** |{3}|{4}{5}", cmdletInfo.FullCommand.Replace("-", "&#8209;"), cmdletInfo.Verb, cmdletInfo.Noun, description, cmdletInfo.Platforms, Environment.NewLine);
                }
            }
