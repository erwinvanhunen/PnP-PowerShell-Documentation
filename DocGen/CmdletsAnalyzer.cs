...
using DocGen.Model;
using CmdletInfo = DocGen.Model.CmdletInfo;
...
namespace DocGen
{
    internal class CmdletsAnalyzer
    {
        ...
        private List<CmdletInfo> GetCmdlets()
        {
            ...
            var types = _assembly.GetTypes().Where(t => t.BaseType != null && (t.BaseType.Name.StartsWith("SPO") || t.BaseType.Name.StartsWith("PnP") || t.BaseType.Name == "PSCmdlet") && !t.BaseType.Namespace.Contains("SPOProxy")).OrderBy(t => t.Name).ToArray();
...
#pragma warning restore CS0612 // Type or member is obsolete
                    ...
                    if (helpAttribute != null)
                    {
                        ...
                        cmdletInfo.Platforms = new List<string>();
                        if (!a.SupportedPlatform.HasFlag(CmdletSupportedPlatform.All))
                        {
                            ...
                            {
                                cmdletInfo.Platforms.Add("SharePoint On-Premises");
                            }
                            if (a.SupportedPlatform.HasFlag(CmdletSupportedPlatform.Online))
                            {
                                cmdletInfo.Platforms.Add("SharePoint Online");
                            }
                            if (a.SupportedPlatform.HasFlag(CmdletSupportedPlatform.SP2013))
                            {
                                cmdletInfo.Platforms.Add("SharePoint 2013");
                            }
                            if (a.SupportedPlatform.HasFlag(CmdletSupportedPlatform.SP2016))
                            {
                                cmdletInfo.Platforms.Add("SharePoint 2016");
                            }
                            //cmdletInfo.Platforms = string.Join(", ", platforms);
                        }
                    ...
                    {
                        var exampleEx = new CmdletExampleAttributeEx()
                        {
                            Code = exampleAttribute.Code,
                            Introduction = exampleAttribute.Introduction,
                            Remarks = exampleAttribute.Remarks,
                            SortOrder = exampleAttribute.SortOrder,
                            Platforms = new List<string>() { _platform }
                        };
                        cmdletInfo.Examples.Add(exampleEx);
                    }
